...
import { ShoppingBagIcon, UserIcon } from './components/Icons.js';
...
const App = () => {
  ...
  const toggleView = () => {
    setViewMode(prev => prev === VIEW_MODES.BUYER ? VIEW_MODES.ADMIN : VIEW_MODES.BUYER);
    if (viewMode === VIEW_MODES.BUYER) setIsCartOpen(false);
  };
        ...
        <div className="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              ...
              <span className="hidden sm:inline">{viewMode === VIEW_MODES.ADMIN ? 'Admin View' : 'Admin Login'}</span>
            </button>
          </div>
        </div>
      </header>
